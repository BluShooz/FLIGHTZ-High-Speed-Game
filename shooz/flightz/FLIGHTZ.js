...
shooz.flightz.FLIGHTZ = function(opts)
{
	var self = this;
	...
	this.track = shooz.flightz.tracks[ opts.track == undefined ? 'Cityscape' : opts.track ];
...
shooz.flightz.FLIGHTZ.prototype.start = function()
{
...
shooz.flightz.FLIGHTZ.prototype.reset = function()
{
	this.manager.get('game').objects.lowFPS = 0;
	this.gameplay.start();

	shooz.Audio.stop('bg');
	shooz.Audio.stop('wind');
	shooz.Audio.volume('wind', 0.35);
	shooz.Audio.play('bg');
	shooz.Audio.play('wind');
}

shooz.flightz.FLIGHTZ.prototype.restart = function()
{
...
shooz.flightz.FLIGHTZ.prototype.update = function()
{
...
shooz.flightz.FLIGHTZ.prototype.init = function()
{
	this.initHUD();
...
shooz.flightz.FLIGHTZ.prototype.load = function(opts)
{
	this.track.load(opts, this.quality);
}

shooz.flightz.FLIGHTZ.prototype.initGameplay = function()
{
	var self = this;

	this.gameplay = new shooz.flightz.Gameplay({
		mode: this.mode,
		hud: this.hud,
...
shooz.flightz.FLIGHTZ.prototype.displayScore = function(f, l)
{
	this.active = false;

	var tf = shooz.Timer.msToTimeString(f);
	var tl = [
		shooz.Timer.msToTimeString(l[0]),
		shooz.Timer.msToTimeString(l[1]),
		shooz.Timer.msToTimeString(l[2])
	];
	...
	if(this.gameplay.result == this.gameplay.results.FINISH)
	{
		...
		// ladder record
		var p = shooz.flightz.Ladder.global[t][d][shooz.flightz.Ladder.global[t][d].length-2];
	...
	st != undefined && (st.href='http://twitter.com/share?text='+encodeURIComponent('I just scored '+dt.innerHTML+' in '+'Cityscape ('+d+') on #FLIGHTZ! Come try it and beat my record on '));
	sf != undefined && (sf.href='http://www.facebook.com/sharer.php?s=100'
		+'&p[title]='+encodeURIComponent('I just scored '+dt.innerHTML+' in '+'Cityscape ('+d+') on FLIGHTZ!')
		+'&p[summary]='+encodeURIComponent('FLIGHTZ is a futuristic racing game built by Thibaut Despoulain (SHOOZ) using HTML5, Javascript and WebGL. Come challenge your friends on this fast-paced 3D game!')
		+'&p[url]='+encodeURIComponent('http://flightz.shooz.com')
		+'&p[images][0]='+encodeURIComponent('http://flightz.shooz.com/image.png'));

	shooz.flightz.Ladder.displayLadder('finish-ladder', t, d, 8);
...
shooz.flightz.FLIGHTZ.prototype.initRenderer = function()
{
	...
	this.manager = new shooz.threejs.RenderManager(renderer);
}

shooz.flightz.FLIGHTZ.prototype.initHUD = function()
{
	if(!this.displayHUD) return;
	this.hud = new shooz.flightz.HUD({
		width: this.width,
		height: this.height,
...
shooz.flightz.FLIGHTZ.prototype.initGameComposer = function()
{
	...
	var effectHex = new THREE.ShaderPass( shooz.threejs.Shaders[ "hexvignette" ] );
...
shooz.flightz.FLIGHTZ.prototype.createMesh = function(parent, geometry, x, y, z, mat)
{
...
shooz.flightz.FLIGHTZ.prototype.tweakShipControls = function()
{
